...
/// <summary>
/// Accounting of movements (kinematics) and of forces (dynamics).
/// </summary>
namespace dyn
{
	/// <summary>
	...
	public:
		...
		/// <summary>
		/// Kinematic and dynamic properties, with possible
		/// accounting for lifecycle management.
		/// </summary>
		struct Entry : public dyn::Entry {};
	...
	public:
		// Try not to clone or move `copy` (copy of the table)
		// because it's only for storage optimizations (save allocations).
		//
		// Don't move `drv` (function table) because I think
		// they're meant to be copied around (by the C++ standard ... or are they?).

		Dyn() = default;
		Dyn(Param const& par) : par(par) {}
		Dyn(Dyn const& dyn)
			: par(dyn.par), tab(dyn.tab), drv(dyn.drv), copy()
		...
		/// <summary>
		/// Access an entry (particle) in the table.
		/// </summary>
